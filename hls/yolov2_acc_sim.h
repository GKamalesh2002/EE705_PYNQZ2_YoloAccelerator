...
#define CONV24_LEN (13*14*1024/2)
	...
	for(x=0;x<18;x++)
	{
		...
		{
			in_ptr[x] = Memory_top;
			out_ptr[x] = Memory_bottom - net->layers[x].out_c * net->layers[x].out_h * out_w_align_256b/2;
		}
		else
		{
	...
	for(x=18;x<25;x++)
	{
		...
		{
			in_ptr[x] = Memory_top;
			out_ptr[x] = Memory_bottom - ROUTE16_LEN - net->layers[x].out_c * net->layers[x].out_h * out_w_align_256b/2;
		}else
		{
			in_ptr[x] = out_ptr[x-1];
			out_ptr[x] = Memory_top;
	...
	out_ptr[30] = Memory_bottom - (net->layers[30].outputs + 13*1*425)/2;//13*14*425/2
...
void yolov2_hls_ps(network *net, float *input)
{
    	...
    	for(int i = 0; i < net->n; ++i)
	{
		...
		if((l.type == CONVOLUTIONAL) || (l.type == MAXPOOL))
		{
			...
			uint32_t en_bits = 0x00000000;//enable_bits[2:0]={IsReLU, LoadBias, IsNotConv}
			if(IsNotConv)
				en_bits |= 0x00000001;
			if(ltype == LT_CONV)
				en_bits |= (0x00000001 << 1);
			switch(l.type)
			{
				case CONVOLUTIONAL:
